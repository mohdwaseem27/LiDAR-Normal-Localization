...
class PoseComparator:
    ...
    def calculate_average_distance(self):
        ...
        if self.record_data:
            self.rmse_values.append((rmse_position, rmse_yaw))
    ...
    def save_to_csv(self):
        ...
        # Check the 'num_particles' parameter
        # num_particles = rospy.get_param('/num_particles', 500)
        # run = rospy.get_param('/run', 1)
        filename = f"/home/waseem/Documents/HBRS/RnD/catkin_ws/src/experiments/new/robustness/circular_room/{algorithm}_{num_particles}_{run}_particles.csv"

        with open(filename, 'w', newline='') as csvfile:
            fieldnames = ['RMSE_Position', 'RMSE_Yaw']
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

            writer.writeheader()
            for rmse_position, rmse_yaw in self.rmse_values:
                writer.writerow({'RMSE_Position': rmse_position, 'RMSE_Yaw': rmse_yaw})
